...
| **Emby** | 挂载 Emby 媒体服务器，浏览媒体库并按剧集/季/单集逐级选择，播放时支持服务器转发与直链直连两种模式 |
| **Jellyfin** | 挂载 Jellyfin 媒体服务器，浏览媒体库并按剧集/季/单集逐级选择，播放时支持服务器转发与直链直连两种模式 |
...
- **Emby / Jellyfin**：在挂载点管理中填写服务器地址与凭据（API Key 或账号密码），连接后可浏览媒体库（电影 / 剧集 / 季 / 单集）逐级选择影片加入房间。

#### Emby / Jellyfin 播放模式

| 模式 | 说明 | 适用场景 |
|---|---|---|
| **服务器转发** | 播放列表中的地址指向本服务的中转端点，由后端携带 Emby/Jellyfin 凭据请求媒体流 | 浏览器无法直接设置 Referer/Authorization 头、媒体服务器未开启直连鉴权的场景 |
| **直链直连** | 播放器直接访问 Emby/Jellyfin 服务器下发的媒体流地址 | 媒体服务器与观众网络可达、无跨域限制的场景，减少服务器中转流量 |

在影片添加面板选择挂载后，可逐级浏览媒体库并批量添加；添加时通过「播放方式」下拉选择服务器转发或直链直连。
