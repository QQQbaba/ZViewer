...
export function selectEngine(source: PlayerSource): PlayerEngine {
  const engineChoice = { format: source.format, audioUrl: !!source.audioUrl, url: source.url?.slice(0, 80), engine: '' }
  // DASH 源或含独立音频轨 → dash.js 引擎
  // （自研 MSE 引擎暂时禁用，统一由 dash.js 处理双轨合并）
  if (source.format === 'dash' || source.audioUrl) {
    engineChoice.engine = 'dash'
    ;(window as unknown as { __engineChoice?: unknown }).__engineChoice = engineChoice
    return ENGINES.dash
  }
